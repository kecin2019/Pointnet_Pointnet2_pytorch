...
class TomatoDataset(Dataset):
    def __init__(
        ...
        rooms = [room for room in rooms if "T0" in room]
        if split == "train":
            rooms_split = [
                room for room in rooms if not "T0{}".format(test_area) in room
            ]
        else:
            rooms_split = [room for room in rooms if "T0{}".format(test_area) in room]

        self.room_points, self.room_labels = [], []
        self.room_coord_min, self.room_coord_max = [], []
        num_point_all = []
        labelweights = np.zeros(43)

        for room_name in tqdm(rooms_split, total=len(rooms_split)):
            room_path = os.path.join(data_root, room_name)
            room_data = np.load(room_path)  # xyzrgbl, N*7
            points, labels = room_data[:, 0:6], room_data[:, 6]  # xyzrgb, N*6; l, N
            tmp, _ = np.histogram(labels, range(44))
            labelweights += tmp
            coord_min, coord_max = (
...
class ScannetDatasetWholeScene:
    ...
    def __init__(
        ...
        if self.split == "train":
            self.file_list = [
                d for d in os.listdir(root) if d.find("T0%d" % test_area) is -1
            ]
        else:
            self.file_list = [
                d for d in os.listdir(root) if d.find("T0%d" % test_area) is not -1
            ]
...
if __name__ == "__main__":
    data_root = "/workspace/dataset/tomato3d/"
    num_point, test_area, block_size, sample_rate = 4096, 7, 1.0, 0.01

    point_data = TomatoDataset(
        split="train",
        data_root=data_root,
        num_point=num_point,
        test_area=test_area,
